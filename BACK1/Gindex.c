...
static int    PostBytes;   //  # of bytes less one needed for a position
static int    Comp[256];   //  DNA complement of packed byte
static int    Select[256]; //  1st byte -> block (of NTHREAD)
...
{ SP *parm = (SP *) args;
  ...
  int    nid  = tid | 0x80;
  int64  nextpost, basepost, anchor;
  uint8 *bend, *btop, *b;
...
//  for 1st byte range [beg,end), find each group of equal k-mers and if < FREQ, then
...
{ RP     *parm  = (RP *) args;
  ...
  uint8 *bed2 = (buf2 + 500000) - (PostBytes+2);
...
#ifdef DEBUG_SORT
if (o == 0x0c && sarray[x+1] == 0x30 && sarray[x+2] == 0xc1) {
        printf(" %lld: %02x %d\n",x,o,w);
        print_table(sarray+x,swide,w);
}
#endif
...
{ DAZZ_DB _DB, *DB = &_DB;
  ...
  { int64 npost, maxp, cum, t;   //  Compute DB split into NTHREADS parts
    int   p, r, len;
    ...
    maxp = 0;
    for (r = 1; r <= NTHREADS; r++)
      if (DBpost[r] - DBpost[r-1] > maxp)
        maxp = DBpost[r] - DBpost[r-1];

    PostBytes = -1;                 //  # of bytes less one for encoding a post
    cum = 1;
    while (cum <= maxp)
      { cum *= 256;
        PostBytes += 1;
